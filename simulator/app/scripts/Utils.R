...
setGeneric("get_side_plot_show",function(obs_tumor_tibble) standardGeneric("get_side_plot_show"))
setMethod("get_side_plot_show",
          signature(obs_tumor_tibble="tbl"),
          function(obs_tumor_tibble){
            
            tryCatch(expr = {
                max_size_reached<-max(obs_tumor_tibble%>%group_by(time)%>%summarise(Ncells=sum(Ncells)))
                latex_label_k<-scales::scientific(max_size_reached, digits = 1)
                latex_label_k<-str_split(latex_label_k,"e",simplify = TRUE)
                if(latex_label_k[1,1]=="1"){latex_label_k[1,1]<-""}
                if(grepl(x=latex_label_k[1,2],pattern="+")){
                  latex_label_k[1,2]<-gsub(x=latex_label_k[1,2],pattern="+","",fixed = TRUE)}
